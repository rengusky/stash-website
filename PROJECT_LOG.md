...
## 2026-09-07 — Mobile round 4 (sticky nav, hero caption, sync nudge, footer STASH clip)
1. **Sticky nav on scroll** (global): `.nav { position: sticky; top: 0; z-index: 50 }` + a `.nav::before` full-viewport-width (`width:100vw; left:calc(50% - 50vw)`) translucent blurred bar (`hsl(var(--background)/.82)` + `backdrop-filter: blur(12px)`) so content scrolls cleanly under it. **Gotcha:** it didn't stick at first because the earlier `overflow-x: hidden` on `body` makes it a scroll container, which breaks descendant `position: sticky`. Fixed by switching the mobile overflow guard to **`overflow-x: clip`** (clips without creating a scroll container) — sticky works and horizontal overflow stays fixed (bodyScrollWidth == viewport).
2. **"Also available for Mac and iPad"** caption added under the phone (`<p class="also-available">`), shown only ≤760px (mobile shows just the iPhone); hidden on desktop where both devices already appear.
3. **Sync asset nudged right 16px** on mobile (`.sync-devices { transform: translateX(16px) }`) — the Mac mockup was inset ~14px vs the design's ~25–30px.
4. **Footer STASH clipped like the web view.** Desktop shows an oversized STASH whose **bottom** is clipped at the footer line, spanning ~full width. Mobile was a small, fully-contained word. Enlarged it (`38vw`), lowered it (`top:64%`), and shortened the closing section (`.join min-height` 350→340) so the word now spans edge-to-edge (S/H just at the edges) with its lower half clipped behind the footer text — matching the desktop treatment. (Full aspect-parity isn't possible on a 390px viewport — matching desktop's tall+full-width letters there means the outermost edges of S/H sit right at the viewport edges.)
...
## 2026-09-09 — Favicon, footer logo, watermark tracking
- **Favicon updated to the new brand.** Replaced the old carried-over `public/assets/favicon.svg` with an app-icon-style mark: a rounded `#E8431F` square + the logo's white "S" glyph (extracted from `App Logo.svg`); legible down to 16px. Added a 180×180 `public/assets/apple-touch-icon.png` (rendered from the SVG) and pointed `apple-touch-icon` at it (iOS prefers PNG); `icon`/`mask-icon` stay on the SVG.
- **Footer wordmark → logo.** Replaced the footer "Stash" text with the inline logo SVG (`.footer-brand`/`.footer-logo`, brand-colored, 18px), matching the nav.
- **Clipped STASH watermark now reads as the title font.** It was already Bricolage (confirmed via `document.fonts` — not a fallback), but `letter-spacing:-.08em` jammed the letters together so it looked like a different condensed face. Loosened to `-.04em` to match the headings; desktop still fits (99→1158), mobile still bottom-clips.

## 2026-09-11 — Favicon raster fallbacks (branch: favicon-fallbacks)
The coral SVG favicon shipped correctly, but the tab still showed an old/default ("blue") icon on some browsers. Root cause: the site offered **only an SVG favicon** and `/favicon.ico` 404'd — browsers that don't render SVG favicons (or that auto-request `/favicon.ico`) fell back to a cached/default icon.
- Generated raster icons from `public/assets/favicon.svg`: `public/favicon.ico` (16/32/48 embedded, built via a small Node ICO encoder — no ImageMagick/PIL available), `public/favicon-32.png`, `public/favicon-16.png`, all at the **root** so `/favicon.ico` resolves.
- `<head>` now offers ICO + SVG + PNG so every browser gets the coral "S". Verified: `/favicon.ico` → 200 image/x-icon, PNGs → image/png, SVG intact; renders correctly at 16/32/48.
- First change made under the new workflow (branch → plan → approve → implement → approve → push); opened as a PR into `main` rather than committing to main directly.

## 2026-09-14 — Mac preview: living feed (branch: mac-preview-scroll)
The hero Mac mockup was a flat static PNG and read as plain. Made its content a slow, continuously scrolling "living feed" (columns drifting, alternating up/down).
- **Assets:** pulled the real card images from Figma (`ZRcbdxA3EPk7AHnt2TV7Zo`, node `710-1042`) via `download_assets` (limit had reset). Got 20 raw images but with visual duplicates (ATLANTIS/Simplicity/Palette/train/Spotify/sunset each appeared twice — same photo re-exported at different sizes, so not byte-identical). Deduped to **14 unique** cards, resized to 480w, saved to `public/assets/mac-feed/` (~2 MB).
- **Structure:** keep the mockup **PNG for the window chrome** (traffic lights, sidebar, search bar); overlay a live content area (`.mac-feed`) positioned by **% insets** over the app's content region (top 5.4% / left 17.6% / right 1.8% / bottom 0). Uses `container-type: inline-size` on `.desktop-app` so all internal spacing/radius is in **cqw** and scales with the mockup at any width.
- **Animation:** 4 masonry columns, each an infinite vertical scroll (`translateY` 0↔-50% over a duplicated card stack → seamless loop), **alternating direction** (up/down/up/down) with varied durations (44–60s) for an organic feel. GPU transforms only.
- **Safety:** `prefers-reduced-motion` → static (caught by the global `*{animation:none}` rule); **desktop/tablet only** (the Mac is `display:none` on mobile, so zero mobile impact). Feed images are `loading=lazy`/`decoding=async`.
- Verified via two headless frames (columns shift between them = motion works), no adjacent duplicates, chrome/shadow/phone-overlap all intact. Built under the branch workflow; PR to follow on approval.

## 2026-09-15 — Mac preview pivot: scroll → drop-in feed (branch: mac-preview-scroll)
The continuous-scroll version (prev entry) "did not look inspiring" — a uniform drift reads as a background loop, not activity. Pivoted to a **living feed where new cards drop into the top of each column one by one**, nudging the rest down (the "new save just landed" moment).
- **Same 14 Figma cards**, split into 4 column pools (`data-pool` on each `.mac-col`). Each column seeds 3–5 static cards (HTML) so the content region is full on load with no white gap, then JS keeps prepending.
- **Drop-in animation:** each new card mounts collapsed (`grid-template-rows: 0fr` + `opacity:0` + slight `translateY/scale` on the img), then opens on the next tick — the `0fr→1fr` grid trick animates height so the card grows in and pushes the column down (spring cubic-bezier). CSS in `interactions.css` (`.mac-card-wrap`/`.enter`).
- **Pacing:** calm and staggered — base gap ~3.2 s + per-column offset + jitter, start delay staggered per column, so columns don't all fire together.
- **Robustness:** `if (document.hidden) return` guard — rAF/compositing pauses in hidden tabs, which would leave cards stuck collapsed (0-height) and pile up; guarding pauses cleanly and resumes when visible. Old cards trimmed by count (>12) then by height (`clientHeight*2.4`) so the DOM stays bounded. The "open" trigger uses `setTimeout` (fires under throttling) rather than rAF.
- **Safety unchanged:** `prefers-reduced-motion` and no-JS → the seeded static grid; desktop/tablet only (Mac is `display:none` ≤760). Feed srcs are root-absolute `/assets/mac-feed/…` (Vite doesn't rewrite runtime JS strings, so `./public/...` would 404 in prod).
- Verified in the in-app browser: initial fill covers the full column (no gap once images load), the drop-in transition animates `0px → full height` with fade, and the hidden-guard correctly pauses the feed. Built under the branch workflow; PR to follow on approval.

## 2026-09-16 — Mac feed: slower cadence + bright, shape-varied cards (branch: mac-preview-scroll)
Polished the drop-in feed after reviewing it live.
- **Cadence slowed:** cards were entering too fast. Base gap ~3.2 s → **~6.5 s** per column (+ per-column offset + up to ~2.2 s jitter), start delays stretched too — each column now drops a card roughly every 6.5–9 s, calmer and less busy (`initMacFeed` in `main.js`).
- **Empty-top fix:** the feed *looked* half-empty at the top even though columns were geometrically full — because the leading cards were pale (e.g. the near-white "ATLANTIS logo clear-space" slide). Diagnosed via DOM measurement (tracks fill from the top, ≥ column height; the gap was pale leads, not real emptiness).
- **7 new bright cards added** (`card_20`–`card_26.webp`, high-contrast color-block logos + rich photos) supplied by the owner. Now **21 cards** total across the 4 column pools.
- **Shape-varied leads:** first pass led every column with a landscape 4:3 webp → the top row read as one repeated wide rectangle. Reordered so the top row **alternates wide → tall → wide → tall**: `card_20` (lime, wide), `card_19` (Dutch floral still-life, tall), `card_23` (blue flower, wide), `card_11` (railroad graphic, tall). Tall leads stagger the masonry naturally. The pale `card_01` is demoted deep into a rotation so it never leads.
- **Fill fix:** the tall-led column 4 fell ~28 px short of the bottom, so added a 5th seed card — all four columns now fill top-to-bottom (verified track height ≥ column height, images loaded).
- Verified in the in-app browser (taller emulated viewport to bring the whole feed on-screen): vibrant, dense, staggered top row; no white gap. Build passes.
